...
$sql = "SELECT * FROM skin_diary WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['rating_skin']) || empty($_POST['rating_appearance'])) {
        ...
        $image_skin = $row['image_skin']; // Keep old image as default
        ...
        if (isset($_FILES['image_skin']) && $_FILES['image_skin']['error'] == 0) {
            ...
            if (in_array($_FILES['image_skin']['type'], $allowed_types)) {
                if ($_FILES['image_skin']['size'] <= $max_size) {
                    $new_image = time() . '_' . basename($_FILES['image_skin']['name']);
                    ...
                    if (move_uploaded_file($_FILES['image_skin']['tmp_name'], $target_file)) {
                        // Delete old image
                        if (!empty($row['image_skin']) && file_exists($target_dir . $row['image_skin'])) {
                            unlink($target_dir . $row['image_skin']);
                        }
                        $image_skin = $new_image;
                    } else {
        ...
        $image_appearance = $row['image_appearance']; // Keep old image as default
        ...
        if (isset($_FILES['image_appearance']) && $_FILES['image_appearance']['error'] == 0) {
            ...
            if (in_array($_FILES['image_appearance']['type'], $allowed_types)) {
                if ($_FILES['image_appearance']['size'] <= $max_size) {
                    $new_image = time() . '_' . basename($_FILES['image_appearance']['name']);
                    ...
                    if (move_uploaded_file($_FILES['image_appearance']['tmp_name'], $target_file)) {
                        // Delete old image
                        if (!empty($row['image_appearance']) && file_exists($target_dir . $row['image_appearance'])) {
                            unlink($target_dir . $row['image_appearance']);
                        }
                        $image_appearance = $new_image;
                    } else {
                        $error = "Failed to upload appearance image.";
                    }
                } else {
                    $error = "Image file is too large. Maximum size is 5MB.";
                }
        ...
        if (empty($error)) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssssssi", $rating_skin, $rating_appearance, $note_skin, $note_appearance, $concerns, $mood_tags, $image_skin, $image_appearance, $id);
            
            if ($stmt->execute()) {
                header("Location: diary.php?updated=1");
                exit;
            } else {
                $error = "Error updating diary: " . $stmt->error;
            }
            $stmt->close();
        }
    }
...
<head>
    ...
    <title>Edit My Diary - FleurSkin</title>
...
<body class=" min-h-screen p-5 lg:m-6 bg-first">
        ...
        <div class="flex justify-center mb-8">
            <h3 class="font-bold text-slate-600 text-4xl">Edit Glow Up Journal</h3>
        </div>     
        ...
        <form method="POST" enctype="multipart/form-data" id="diaryForm">
            <div class="lg:flex w-full max-h-min mt-4 gap-5 items-stretch">
                <div class="p-4 bg-white rounded-2xl shadow-xl lg:w-1/2 h-1/2">
                    ...
                    <!-- Rating Section -->
                        <div>
                            ...
                            <div class="flex gap-2 mb-4">
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <span class="star-rating cursor-pointer material-symbols-outlined text-3xl text-gray-300 <?php echo ($i <= $row['rating_skin']) ? 'active' : ''; ?>" 
                                          data-rating="<?php echo $i; ?>" data-form="skin">star</span>
                                <?php endfor; ?>
                            </div>
                            <input type="hidden" name="rating_skin" id="rating_skin" value="<?php echo htmlspecialchars($row['rating_skin']); ?>" required>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                <input class="w-full border border-slate-300 rounded-md p-2 text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500" 
                                    type="text" 
                                    name="concerns" 
                                    value="<?php echo isset($_POST['concerns']) ? htmlspecialchars($_POST['concerns']) : htmlspecialchars($row['concerns']); ?>"
                                    >
                    ...
                    <!-- Current and add new image -->
                        <div class="mt-4">
                            <?php if (!empty($row['image_skin'])): ?>
                                <div class="mb-3">
                                    <label class="block text-gray-700 font-medium mb-2">Current Skin Image:</label>
                                    <img src="uploads_diary1/<?php echo htmlspecialchars($row['image_skin']); ?>" 
                                         alt="Current skin image" 
                            ...
                            <small class="text-slate-500 text-xs mt-1 block">Leave empty to keep current image</small>
                        </div>
                    ...
                    <!-- Notes -->
                        <div class="relative mt-5">
                            <textarea name="note_skin" rows="4" 
                                      ...
                                      placeholder="Write about your skin journey today..."><?php echo isset($_POST['note_skin']) ? htmlspecialchars($_POST['note_skin']) : htmlspecialchars($row['note_skin']); ?></textarea>
                        </div>
                ...
                <div id="mood" class="p-4 mt-10 lg:mt-0 bg-white rounded-2xl shadow-xl lg:w-1/2 h-1/2">
                    ...
                    <!-- Rating Section -->
                        <div>
                            ...
                            <div class="flex gap-2 mb-4">
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <span class="star-rating cursor-pointer material-symbols-outlined text-3xl text-gray-300 <?php echo ($i <= $row['rating_appearance']) ? 'active' : ''; ?>" 
                                          data-rating="<?php echo $i; ?>" data-form="appearance">star</span>
                                <?php endfor; ?>
                            </div>
                            <input type="hidden" name="rating_appearance" id="rating_appearance" value="<?php echo htmlspecialchars($row['rating_appearance']); ?>" required>
                        </div>
                    ...
                    <!-- Mood Tags -->
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                // Convert saved mood tags to array for checking
                                $saved_moods = !empty($row['mood_tags']) ? explode(', ', $row['mood_tags']) : [];
                                
                                foreach($moods as $mood): 
                                    $is_checked = in_array($mood['label'], $saved_moods);
                                ?>
                                    <label class="mood-tag inline-flex items-center space-x-2 cursor-pointer">
                                        <input type="checkbox" name="mood_tags[]" value="<?php echo $mood['label']; ?>" <?php echo $is_checked ? 'checked' : ''; ?>>
                    ...
                    <!-- Current and add new image -->
                        <div class="mt-4">
                            <?php if (!empty($row['image_appearance'])): ?>
                                <div class="mb-3">
                                    <label class="block text-gray-700 font-medium mb-2">Current Appearance Image:</label>
                                    <img src="uploads_diary2/<?php echo htmlspecialchars($row['image_appearance']); ?>" 
                                         alt="Current appearance image" 
                                         class="w-32 h-32 object-cover rounded-lg border">
                                </div>
                            <?php endif; ?>
                    ...
                    <!-- Notes -->
                        <div class="relative mt-5">
                            <textarea name="note_appearance" rows="5" 
                                      class="w-full p-4 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-600"
                                      placeholder="Write about your appearance today..."><?php echo isset($_POST['note_appearance']) ? htmlspecialchars($_POST['note_appearance']) : htmlspecialchars($row['note_appearance']); ?></textarea>
                        </div>
                </div>
            ...
            <div class="mt-5 flex gap-2 justify-end">
                ...
                <button type="submit" class="border border-slate-500 text-center bg-[#e6aab4] text-white font-medium py-2 px-6 rounded-3xl hover:bg-[#eb9fab] transition-colors cursor-pointer">Update</button>
            </div>
        </form>

    <script>
        // Initialize star ratings based on existing data
        document.addEventListener('DOMContentLoaded', function() {
            // Set skin rating stars
            const skinRating = document.getElementById('rating_skin').value;
            if (skinRating) {
                const skinStars = document.querySelectorAll('.star-rating[data-form="skin"]');
                for(let i = 0; i < skinRating; i++) {
                    skinStars[i].style.color = '#f59e0b';
                }
            }
            
            // Set appearance rating stars
            const appearanceRating = document.getElementById('rating_appearance').value;
            if (appearanceRating) {
                const appearanceStars = document.querySelectorAll('.star-rating[data-form="appearance"]');
                for(let i = 0; i < appearanceRating; i++) {
                    appearanceStars[i].style.color = '#f59e0b';
                }
            }
        });
